...
export function PhoneInput({ value, onChangeText, label = 'Phone number', error }: PhoneInputProps) {
  return (
    <View className="mb-4">
      <Text className="font-work-sans-medium text-sm text-of-text mb-1.5">{label}</Text>
      <View className="flex-row items-center bg-white border border-of-border rounded-input overflow-hidden">
        <View className="bg-of-pale px-3 py-3.5 border-r border-of-border">
          <Text className="font-work-sans-medium text-sm text-of-text">+255</Text>
        </View>
        <TextInput
          value={value}
          onChangeText={(text) => {
          ...
          maxLength={9}
          className="flex-1 px-3 py-3.5 font-work-sans text-sm text-of-text"
        />
      </View>
