...
import {
  ...
  isValidSocialHref,
  socialLinks,
  type StudioSocialIconKey,
} from '@/lib/navConfig';

function SocialIcon({ iconKey }: { iconKey: StudioSocialIconKey }) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20">
            <div>
              ...
              <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tighter text-white leading-[0.9] mb-8">
                LET&apos;S WORK<br />
                <span className="stroke-text stroke-text-light stroke-fill-none">TOGETHER.</span>
              </h2>
            ...
            <nav className="flex flex-wrap gap-x-6 gap-y-2">
              {footerQuickLinks.map((link) => (
                <Link
                  key={link.id}
                  href={link.href}
                  className="text-[10px] font-mono text-white/30 uppercase tracking-widest hover:text-brand-accent transition-colors focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-accent/50"
                >
                  {link.label}
                </Link>
              ))}
            ...
            <div className="flex gap-6">
              {legalLinks.map((link) => (
                <Link
                  key={link.id}
                  href={link.href}
                  className="text-[10px] text-white/20 font-mono hover:text-white/50 transition-colors tracking-wide focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-accent/50"
                >
                  {link.label}
                </Link>
              ))}
