...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try { await requireStudioRole('studio_viewer'); const { id } = await params;
    const { data, error } = await getStudioSupabaseAdmin().from('studio_faqs').select('*').eq('id', id).single();
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try { await requireStudioRole('studio_editor'); const { id } = await params; const body = await req.json();
    const { data, error } = await getStudioSupabaseAdmin().from('studio_faqs').update(body).eq('id', id).select().single();
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try { await requireStudioRole('studio_admin'); const { id } = await params;
    const { error } = await getStudioSupabaseAdmin().from('studio_faqs').delete().eq('id', id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
