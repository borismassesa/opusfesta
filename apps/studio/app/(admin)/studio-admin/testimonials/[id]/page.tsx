...
export default function EditTestimonialPage() {
  ...
  const [testimonial, setTestimonial] = useState<StudioTestimonial | null>(null);
  const [showDelete, setShowDelete] = useState(false);
  const [deleting, setDeleting] = useState(false);

  useEffect(() => {
    fetch(`/api/studio-admin/testimonials/${id}`)
      .then((r) => r.json())
      .then((d) => setTestimonial(d.testimonial));
  }, [id]);

  const handleSubmit = async (data: Record<string, unknown>) => {
    await fetch(`/api/studio-admin/testimonials/${id}`, {
      method: 'PUT',
  ...
  const handleDelete = async () => {
    setDeleting(true);
    await fetch(`/api/studio-admin/testimonials/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDeleteModal
        ...
        title="Delete Testimonial" description="This will permanently delete this testimonial. This action cannot be undone." loading={deleting}
      />
    </div>
  );
}
