...
// Live camera capture for a single photo. Opens the device camera via
// getUserMedia, lets the user snap a still, preview it, and retake or confirm.
// No file picker — capture only. Returns a downscaled JPEG data URL via
// `onCapture`. getUserMedia needs a secure context (HTTPS or localhost); when
// the camera is unavailable/blocked we surface a clear message so the caller
// can steer the user to the QR-to-phone option.
...
export default function CameraCapture({
  ...
  useEffect(() => {
    // Defer to a timer so the initial state resets inside start() don't run
    // synchronously during the effect (avoids cascading-render warnings).
    const t = setTimeout(() => {
      void start()
    }, 0)
    return () => {
