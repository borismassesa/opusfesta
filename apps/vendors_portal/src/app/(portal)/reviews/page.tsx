...
import {
  reviewInviteCandidates,
  sampleReviews,
  type Review,
  type ReviewInviteCandidate,
} from '@/lib/mock-data'
...
// Local replies map keyed by review id. Backed by component state — when the
// API lands the reviews array (and replies) will come from the server.
type ReplyState = Record<string, { body: string; repliedAt: string } | null>

export default function ReviewsPage() {
  const [reviews, setReviews] = useState<Review[]>(sampleReviews)
  const [pendingInvites, setPendingInvites] = useState<ReviewInviteCandidate[]>(
    reviewInviteCandidates,
  )
  ...
  const visible = useMemo(() => {
    ...
    // Always float pinned to the top regardless of sort.
  ...
  return (
    <div className="px-6 lg:px-10 pt-4 lg:pt-5 pb-12">
      <div className="max-w-6xl space-y-6">
        {/* Top: stats + invite candidates side-by-side */}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] p-6 lg:p-7">
      ...
      <p className="mt-5 text-xs text-gray-500 leading-relaxed">
        OpusFesta auto-collects reviews from couples after each event — you can’t write or
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">
            Past bookings that haven’t left a review yet. We send the couple a polite,
            OpusFesta-branded email — your storefront’s rating only updates if they post.
          </p>
        </div>
      </div>
...
function Stars({
  ...
  // Use floor for solid stars; partial fills via overlay for half-precision.
  return (
