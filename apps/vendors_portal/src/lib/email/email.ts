// Resend transactional email wrapper for vendors_portal. Mirrors
// apps/opus_admin/src/lib/email.ts so behavior stays consistent across apps.
// When RESEND_API_KEY is unset we no-op rather than throw, so submission
// never fails because of an email-config issue.
