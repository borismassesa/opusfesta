...
// A subject is a mail header, not HTML. Strip CR/LF/tab and other control
// chars so a crafted value (e.g. a vendor name containing "\r\nBcc: …") cannot
// inject extra headers. Collapse to spaces rather than escaping — escaping
// would surface literal "&amp;" in the subject line.
function safeHeader(value: string): string {
  // Replace any run of control chars (incl. CR/LF) with a single space, then trim.
...
export async function notifyBookingEventEmail(input: BookingEmailInput): Promise<void> {
  ...
  const subject = safeHeader(`${input.vendorName}: ${input.eventLabel}`)
  const html = `
...
export async function notifyLeadEventEmail(input: LeadEmailInput): Promise<void> {
  ...
  const subject = safeHeader(`${input.vendorName}: ${input.statusLabel}`)
  const html = `
    <div style="font-family: Arial, sans-serif; color: #1f2937; line-height: 1.5;">
      <p>Hi ${safeName},</p>
