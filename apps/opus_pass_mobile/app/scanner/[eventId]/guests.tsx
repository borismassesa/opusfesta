            ...
            return (
              <Pressable
                ...
                className={`rounded-full px-4 py-2 ${
                  active ? 'bg-ed-primary-container' : 'border border-ed-outline-variant bg-ed-surface'
                }`}
              >
                <Text
                  className={`font-work-sans-medium text-xs capitalize ${
      ...
      {rosterQuery.isPending ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View className="mb-3 flex-row items-center justify-between rounded-2xl border border-ed-outline-variant bg-ed-surface p-4">
                <View className="flex-1 pr-3">
                  <View className="flex-row items-center gap-2">
                    <Text className="font-work-sans-bold text-sm text-ed-on-surface" numberOfLines={1}>
                      {item.fullName}
                    </Text>
                    {item.isVip ? (
                      <View className="rounded-full px-2 py-0.5" style={{ backgroundColor: '#9FE870' }}>
                        <Text className="font-work-sans-bold text-[9px] uppercase text-[#1A1A1A]">
                          VIP
                        </Text>
                      </View>
                    ) : null}
                ...
                {arrived ? (
                  ...
                  <Pressable
                    accessibilityRole="button"
                    disabled={admitting === item.invitationId}
                    onPress={() => void admit(item)}
                    className="rounded-full bg-ed-primary-container px-4 py-2.5"
                    style={{ opacity: admitting === item.invitationId ? 0.5 : 1 }}
                  >
                    {admitting === item.invitationId ? (
