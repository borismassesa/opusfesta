...
import { updatePledgePageConfig } from '@/lib/dashboard/actions'
import {
  resolvePledgePage,
...
export default function PledgeCustomizeClient({
  pledgeToken,
  initialConfig,
}: {
  ...
  const [cfg, setCfg] = useState<PledgePageConfig>(() => resolvePledgePage(initialConfig))
  const [device, setDevice] = useState<PreviewDevice>('desktop')
  const [pending, startTransition] = useTransition()

  const [origin, setOrigin] = useState('')
  useEffect(() => setOrigin(window.location.origin), [])
  const previewUrl = pledgeToken && origin ? `${origin}/pledge/${pledgeToken}` : null
  ...
  // Push the latest config whenever it changes (real-time preview).
  useEffect(() => {
    postConfig(cfg)
  }, [cfg, postConfig])

  // The previewed page announces when it's ready; reply with the current config.
  useEffect(() => {
  ...
  function save() {
    startTransition(async () => {
      try {
        await updatePledgePageConfig(cfg)
        toast.success('Pledge page saved')
      } catch (err) {
      ...
      <div className="grid lg:grid-cols-[minmax(0,440px)_1fr]">
        ...
        <div className="border-b border-black/[0.06] lg:h-[calc(100vh-0px)] lg:overflow-y-auto lg:border-b-0 lg:border-r">
          ...
          <div className="px-5 py-6">
            <h1 className="font-serif text-2xl text-[#1A1A1A]">Customize your pledge page</h1>
            <p className="mt-1 text-sm text-[#1A1A1A]/55">
              Edit the wording and look on the left — the preview updates as you type.
            </p>
            <div className="mt-6">
              <PageConfigFields cfg={cfg} setCfg={setCfg} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#F3F1EE]/60 lg:sticky lg:top-0 lg:h-[calc(100vh-0px)]">
          ...
          <div className="flex justify-center overflow-auto p-4 lg:h-[calc(100vh-49px)]">
            {previewUrl ? (
              ...
              <p className="mt-10 text-sm text-[#1A1A1A]/50">No pledge link yet.</p>
            )}
          </div>
        </div>
      </div>
