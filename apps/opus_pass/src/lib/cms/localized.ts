...
//
// This module is PURE — no `next/headers`, no server-only imports — so it is
// safe to import (the values, not just the types) from both server loaders and
// client components, unlike the loader modules guarded by the import-type
// boundary rule.
//
