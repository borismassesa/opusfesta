...
export function SaveTheDatePhoto({
  ...
  return (
    ...
    >
      <defs>
        <clipPath id="std-photo-clip">
          <rect x="-8.5" y="-8.5" width="436.54" height="612.29" />
        </clipPath>
        {/* Diagonal stripe pattern shown when no photo is uploaded */}
        <pattern id="std-placeholder" patternUnits="userSpaceOnUse" width="12" height="12" patternTransform="rotate(45)">
          <rect width="12" height="12" fill="var(--iv-bg)" fillOpacity="0.4" />
          <line x1="0" y1="0" x2="0" y2="12" stroke="var(--iv-acc)" strokeWidth="4" strokeOpacity="0.25" />
        </pattern>
      </defs>
      ...
      {photoSrc ? (
        <image
          ...
          clipPath="url(#std-photo-clip)"
        />
      ) : (
        <rect x="-8.5" y="-8.5" width="436.54" height="612.28" fill="url(#std-placeholder)" />
      )}

      {/* ── Colour overlay — opacity controlled by user ── */}
      <rect x="-8.5" y="-8.5" width="436.54" height="612.28" fill="var(--iv-bg)" fillOpacity={photoOpacity} />
      ...
      {/* When no photo, show a camera icon hint in the window */}
      {!photoSrc && (
        <text x="209.76" y="196" textAnchor="middle" dominantBaseline="middle" fontSize="32" fill="var(--iv-tp)" fillOpacity={0.4}>
          📷
        </text>
      )}
      ...
      {/* ── Date ── */}
      <text transform="translate(181.86 438.76)" fontFamily="'Josefin Sans','Trebuchet MS',system-ui,sans-serif" fontSize="15.98" fill="var(--iv-tp)">
        <tspan x="0" y="0">{date}</tspan>
      </text>

      {/* ── Venue ── */}
      <text transform="translate(134.53 455.73)" fontFamily="'Josefin Sans','Trebuchet MS',system-ui,sans-serif" fontSize="13" fill="var(--iv-tp)">
        <tspan x="0" y="0">{venue}</tspan>
      </text>

      {/* ── Time ── */}
      {time && (
        <text transform="translate(172 472)" fontFamily="'Josefin Sans','Trebuchet MS',system-ui,sans-serif" fontSize="12" fill="var(--iv-mut)">
      ...
      {/* ── Reception ── */}
      {hasReception && (
        <text transform="translate(120 490)" fontFamily="'Josefin Sans','Trebuchet MS',system-ui,sans-serif" fontSize="11" fill="var(--iv-mut)">
          <tspan x="0" y="0">
