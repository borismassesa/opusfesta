  ...
  return (
    ...
    >
      ...
      <g data-section="names">
        {line2 ? (
          <>
            <text x={na.x} y="184" textAnchor={na.textAnchor} dominantBaseline="middle" style={{ ...font.namesStyle, ...(na.fontStyle && { fontStyle: na.fontStyle }) }} fontSize={na.fontSize} fontWeight={na.fontWeight} fill={na.fill ?? 'var(--iv-tp)'} opacity={na.opacity}>{na.uppercase ? line1.toUpperCase() : line1}</text>
      ...
      <text data-section="date" x={da.x} y={line2 ? '240' : '230'} textAnchor={da.textAnchor} dominantBaseline="middle" fontFamily="inherit" fontSize={da.fontSize} fontWeight={da.fontWeight} fontStyle={da.fontStyle} letterSpacing={da.letterSpacing ?? 2.2} fill={da.fill ?? 'var(--iv-ts)'} opacity={da.opacity}>{da.uppercase === false ? date : date.toUpperCase()}</text>
      {venue && (
        <text data-section="venue" x={va.x} y={line2 ? '252' : '242'} textAnchor={va.textAnchor} dominantBaseline="middle" fontFamily="inherit" fontSize={va.fontSize} fontWeight={va.fontWeight} fontStyle={va.fontStyle} fill={va.fill ?? 'var(--iv-ts)'} opacity={va.opacity}>{va.uppercase ? venue.toUpperCase() : venue}</text>
      )}
      {message && (
        <g data-section="message">
          <line x1="100" y1="300" x2="200" y2="300" stroke="var(--iv-acc)" strokeWidth="0.5" strokeOpacity="0.6" />
          <text x={ma.x} y="314" textAnchor={ma.textAnchor} dominantBaseline="middle" style={{ ...font.bodyStyle, ...(ma.fontStyle && { fontStyle: ma.fontStyle }) }} fontSize={ma.fontSize} fontWeight={ma.fontWeight} fill={ma.fill ?? 'var(--iv-ts)'} opacity={ma.opacity}>{ma.uppercase ? message.toUpperCase() : message}</text>
