  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-[15px] font-semibold text-gray-900">Articles</h3>
          <p className="text-sm text-gray-500 mt-1">
            {posts.length} article{posts.length === 1 ? '' : 's'} — edit, publish, or feature any row.
          </p>
        </div>
        <Link
          href="/cms/advice-and-ideas/posts/new"
      ...
      <form
        className="flex flex-wrap items-center gap-2 bg-white border border-gray-100 rounded-2xl px-4 py-3 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]"
        action="/cms/advice-and-ideas/posts"
      >
        <input
          type="text"
          name="q"
          ...
          className="flex-1 min-w-[220px] px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A0DC] focus:border-transparent"
        />
        <select
          name="section"
          defaultValue={section}
          className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A0DC]"
        >
        ...
        <button
          type="submit"
          className="text-sm font-semibold text-[#7E5896] hover:text-[#5c3f72] px-3 py-2 rounded-lg hover:bg-[#F8F0FB]"
        >
          Filter
        </button>
      </form>
          ...
          <tbody className="divide-y divide-gray-100">
            {posts.map((p) => (
              <tr key={p.id} className="hover:bg-gray-50/60 align-middle">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <Link
                        href={`/cms/advice-and-ideas/posts/${p.id}`}
                ...
                <td className="px-4 py-3 text-gray-700 whitespace-nowrap truncate" title={p.section_id}>
                  {formatSectionLabel(p.section_id)}
                </td>
                <td className="px-4 py-3 text-gray-700 whitespace-nowrap truncate" title={p.author_name ?? '—'}>
                  {p.author_name ?? '—'}
                </td>
                <td className="px-4 py-3 text-gray-500 whitespace-nowrap">
                  <div>{formatPublishedDate(p.published_at)}</div>
                  <div className="text-xs text-gray-400">{formatReadTime(p.read_time)}</div>
                </td>
                <td className="px-4 py-3">
                  <span
                    className={
