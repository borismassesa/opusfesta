...
import type { AttireStandoutItem, AttireStandoutStylesContent } from '@/lib/cms/attire-standout-styles'
import { useEditorActions } from '../EditorActionsContext'
import { discardAttireStandoutStylesDraft, publishAttireStandoutStyles, saveAttireStandoutStylesDraft } from './actions'
...
type Props = { initial: AttireStandoutStylesContent; hasDraft: boolean }

export default function StandoutStylesEditor({ initial, hasDraft: initialHasDraft }: Props) {
  const [draft, setDraft] = useState<AttireStandoutStylesContent>(initial)
  ...
  const updateItem = (id: string, patch: Partial<AttireStandoutItem>) =>
  ...
  const addItem = () =>
    setDraft((d) => ({
      ...d,
      items: [...d.items, { id: `style-${Date.now()}`, name: 'New style', discount: 'up to 20% off', img: '' }],
    }))
  ...
  const handleSaveDraft = () => runAction(async () => { await saveAttireStandoutStylesDraft(draft); setHasDraft(true); setMessage('Draft saved.') })
  const handlePublish = () => runAction(async () => { await saveAttireStandoutStylesDraft(draft); await publishAttireStandoutStyles(); setHasDraft(false); setMessage('Published — changes are live.') })
  const handleDiscard = () => runAction(async () => { await discardAttireStandoutStylesDraft(); setDraft(initial); setHasDraft(false); setMessage('Draft discarded.') })
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] space-y-5">
        <h3 className="text-[15px] font-semibold text-gray-900">Standout Styles</h3>
        ...
        <Field label="Section heading">
          <input
            ...
            className={inputCls}
            placeholder="Save now on standout styles"
          />
        </Field>
            ...
            <div key={item.id} className="border border-gray-200 rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <Field label="Discount text">
                  <input type="text" value={item.discount} onChange={(e) => updateItem(item.id, { discount: e.target.value })} className={inputCls} placeholder="up to 20% off" />
                </Field>
              ...
              {item.img && (
                <div className="rounded-xl overflow-hidden border border-gray-200 aspect-square max-w-[80px]">
