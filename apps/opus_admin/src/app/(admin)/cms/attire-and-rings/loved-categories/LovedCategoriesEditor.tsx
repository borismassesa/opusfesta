...
import { discardAttireLovedCategoriesDraft, publishAttireLovedCategories, saveAttireLovedCategoriesDraft } from './actions'
...
export default function LovedCategoriesEditor({ initial, hasDraft: initialHasDraft }: Props) {
  ...
  const handleSaveDraft = () => runAction(async () => { await saveAttireLovedCategoriesDraft(draft); setHasDraft(true); setMessage('Draft saved.') })
  const handlePublish = () => runAction(async () => { await saveAttireLovedCategoriesDraft(draft); await publishAttireLovedCategories(); setHasDraft(false); setMessage('Published — changes are live.') })
  const handleDiscard = () => runAction(async () => { await discardAttireLovedCategoriesDraft(); setDraft(initial); setHasDraft(false); setMessage('Draft discarded.') })
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] space-y-5">
        <h3 className="text-[15px] font-semibold text-gray-900">Most-Loved Categories</h3>
        ...
        <Field label="Section title">
          <input
            ...
            className={inputCls}
            placeholder="Shop our most-loved categories"
          />
        </Field>
          ...
          {draft.items.map((item, idx) => (
            <div key={item.id} className="border border-gray-200 rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Name">
                  <input type="text" value={item.name} onChange={(e) => updateItem(item.id, { name: e.target.value })} className={inputCls} placeholder="Wedding Dresses" />
                </Field>
