...
export async function saveAttireCategoriesDraft(draft: AttireCategoriesContent): Promise<void> {
  ...
  revalidatePath('/cms/attire-and-rings/categories')
}

export async function publishAttireCategories(): Promise<void> {
  const supabase = createSupabaseAdminClient()
...
export async function discardAttireCategoriesDraft(): Promise<void> {
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
