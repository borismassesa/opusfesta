...
import type { AttireAccessoriesContent, AttireAccessoryItem } from '@/lib/cms/attire-accessories'
import { useEditorActions } from '../EditorActionsContext'
import { discardAttireAccessoriesDraft, publishAttireAccessories, saveAttireAccessoriesDraft } from './actions'
...
type Props = { initial: AttireAccessoriesContent; hasDraft: boolean }

export default function AccessoriesEditor({ initial, hasDraft: initialHasDraft }: Props) {
  const [draft, setDraft] = useState<AttireAccessoriesContent>(initial)
  ...
  const updateItem = (id: string, patch: Partial<AttireAccessoryItem>) =>
  ...
  const addItem = () =>
    setDraft((d) => ({
      ...d,
      items: [...d.items, { id: `item-${Date.now()}`, name: 'New accessory', img: '' }],
    }))
  ...
  const handleSaveDraft = () => runAction(async () => { await saveAttireAccessoriesDraft(draft); setHasDraft(true); setMessage('Draft saved.') })
  const handlePublish = () => runAction(async () => { await saveAttireAccessoriesDraft(draft); await publishAttireAccessories(); setHasDraft(false); setMessage('Published — changes are live.') })
  const handleDiscard = () => runAction(async () => { await discardAttireAccessoriesDraft(); setDraft(initial); setHasDraft(false); setMessage('Draft discarded.') })
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] space-y-5">
        <h3 className="text-[15px] font-semibold text-gray-900">Accessories</h3>
        ...
        <Field label="Section heading">
          <input
            ...
            className={inputCls}
            placeholder="Accessories to complete the look"
          />
        </Field>

        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-600">Accessory pills</span>
          ...
          {draft.items.map((item, idx) => (
            <div key={item.id} className="border border-gray-200 rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Name">
                  <input type="text" value={item.name} onChange={(e) => updateItem(item.id, { name: e.target.value })} className={inputCls} placeholder="Bridal Veils" />
                </Field>
              ...
              {item.img && (
                <div className="rounded-full overflow-hidden border border-gray-200 w-14 h-14">
