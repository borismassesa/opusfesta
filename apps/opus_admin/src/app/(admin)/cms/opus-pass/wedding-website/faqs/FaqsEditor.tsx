...
import type {
  OpusPassWebsitesFaqItem,
  OpusPassWebsitesFaqsContent,
...
import {
  discardOpusPassWebsitesFaqsDraft,
  publishOpusPassWebsitesFaqs,
  saveOpusPassWebsitesFaqsDraft,
} from './actions'

type Props = {
  initial: OpusPassWebsitesFaqsContent
  hasDraft: boolean
}
...
export default function FaqsEditor({ initial, hasDraft: initialHasDraft }: Props) {
  const [draft, setDraft] = useState<OpusPassWebsitesFaqsContent>(initial)
  ...
  const setField = <K extends keyof OpusPassWebsitesFaqsContent>(
    key: K,
    value: OpusPassWebsitesFaqsContent[K],
  ) => setDraft((d) => ({ ...d, [key]: value }))

  const setItem = (idx: number, patch: Partial<OpusPassWebsitesFaqItem>) =>
    setDraft((d) => ({
      ...d,
  ...
  const handleSaveDraft = () =>
    runAction(async () => {
      await saveOpusPassWebsitesFaqsDraft(draft)
      setHasDraft(true)
  ...
  const handlePublish = () =>
    runAction(async () => {
      await saveOpusPassWebsitesFaqsDraft(draft)
      await publishOpusPassWebsitesFaqs()
      setHasDraft(false)
  ...
  const handleDiscard = () =>
    runAction(async () => {
      await discardOpusPassWebsitesFaqsDraft()
      setDraft(initial)
      setHasDraft(false)
            ...
            {draft.items.map((item) => (
              <div key={item.id} className="border-b border-gray-200 last:border-b-0 py-3">
                <p className="text-xs font-medium text-gray-900 leading-snug">
                  {resolveLocalized(item.question, previewLocale)}
                </p>
              </div>
            ))}
          </div>
