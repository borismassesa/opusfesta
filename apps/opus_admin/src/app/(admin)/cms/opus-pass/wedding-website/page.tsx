...
const sections: SectionCard[] = [
  {
    key: 'hero',
    label: 'Hero',
    description: 'Top banner pitch — beautiful wedding site in minutes, one link.',
    icon: Sparkles,
  },
  {
    key: 'templates',
    label: 'Templates Gallery',
    description: 'Curated wedding website templates with previews.',
    icon: LayoutTemplate,
  },
  {
    key: 'photo-gallery',
    label: 'Photo Gallery Feature',
    description: 'Showcase of how couples build their gallery — story timeline, captions, uploads.',
    icon: Image,
  },
  {
    key: 'custom-link',
    label: 'Custom Link',
    description: 'Promo for the personalised yourname.opuspass.events link.',
    icon: Link2,
  },
  {
    key: 'travel-info',
    label: 'Travel & Venue',
    description: 'Demo of how venue, accommodation and travel info appears on the live wedding site.',
    icon: PlaneTakeoff,
  },
  {
    key: 'info',
    label: 'About Wedding Websites',
    description: 'Closing copy block — what couples get and how the website integrates with invites & RSVPs.',
    icon: Info,
  },
]

export default function OpusPassWeddingWebsiteCmsPage() {
  const opusPassUrl = process.env.NEXT_PUBLIC_OPUS_PASS_URL ?? 'http://localhost:3008'
  return (
    <div className="px-8 py-6">
      <div className="flex items-start justify-between gap-6 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Wedding Website</h1>
          <p className="text-sm text-gray-600">
            Sections on the OpusPass wedding website page. Editors are coming online section by section.
          </p>
        </div>
        <Link
