...
const SECTION_KEY = 'manifesto'

export async function saveOpusPassHomepageManifestoDraft(
  draft: OpusPassHomepageManifestoContent,
): Promise<void> {
  ...
  revalidatePath('/cms/opus-pass/homepage/manifesto')
}

export async function publishOpusPassHomepageManifesto(): Promise<void> {
  const supabase = createSupabaseAdminClient()
...
export async function discardOpusPassHomepageManifestoDraft(): Promise<void> {
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
