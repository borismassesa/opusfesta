...
import type { OpusPassHomepageShowcaseContent } from '@/lib/cms/opus-pass-homepage-showcase'

const PAGE_KEY = 'opus-pass-homepage'
const SECTION_KEY = 'showcase'

export async function saveOpusPassHomepageShowcaseDraft(
  draft: OpusPassHomepageShowcaseContent,
): Promise<void> {
  ...
  revalidatePath('/cms/opus-pass/homepage/showcase')
}

export async function publishOpusPassHomepageShowcase(): Promise<void> {
...
export async function discardOpusPassHomepageShowcaseDraft(): Promise<void> {
  await requirePermission('cms.write')
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
