...
import type { OpusPassInfoContent } from '@/lib/cms/opus-pass-info'

const PAGE_KEY = 'opus-pass-homepage'
const SECTION_KEY = 'info'

export async function saveOpusPassInfoDraft(draft: OpusPassInfoContent): Promise<void> {
  ...
  revalidatePath('/cms/opus-pass/homepage/info')
}

export async function publishOpusPassInfo(): Promise<void> {
  const supabase = createSupabaseAdminClient()
...
export async function discardOpusPassInfoDraft(): Promise<void> {
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
