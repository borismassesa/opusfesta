...
export async function saveOpusPassHeroDraft(draft: OpusPassHeroContent): Promise<void> {
  ...
  revalidatePath('/cms/opus-pass/homepage/hero')
}

export async function publishOpusPassHero(): Promise<void> {
  const supabase = createSupabaseAdminClient()
...
export async function discardOpusPassHeroDraft(): Promise<void> {
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
