...
import type { OpusPassFeaturesContent } from '@/lib/cms/opus-pass-features'

const PAGE_KEY = 'opus-pass-homepage'
const SECTION_KEY = 'features'

export async function saveOpusPassFeaturesDraft(draft: OpusPassFeaturesContent): Promise<void> {
  ...
  revalidatePath('/cms/opus-pass/homepage/features')
}

export async function publishOpusPassFeatures(): Promise<void> {
...
export async function discardOpusPassFeaturesDraft(): Promise<void> {
  await requirePermission('cms.write')
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
