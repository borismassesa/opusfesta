...
import type { OpusPassGuestsFaqsContent } from '@/lib/cms/opus-pass-guests-faqs'

const PAGE_KEY = 'opus-pass-guests'
const SECTION_KEY = 'faqs'

export async function saveOpusPassGuestsFaqsDraft(
  draft: OpusPassGuestsFaqsContent,
): Promise<void> {
  ...
  revalidatePath('/cms/opus-pass/guests-rsvps/faqs')
}

export async function publishOpusPassGuestsFaqs(): Promise<void> {
...
export async function discardOpusPassGuestsFaqsDraft(): Promise<void> {
  await requirePermission('cms.write')
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
