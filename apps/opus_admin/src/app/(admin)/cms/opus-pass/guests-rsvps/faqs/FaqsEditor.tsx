...
import {
  ...
  publishOpusPassGuestsFaqs,
  saveOpusPassGuestsFaqsDraft,
} from './actions'
...
function randomId(): string {
  return `gfaq-${Math.random().toString(36).slice(2, 9)}`
}

export default function FaqsEditor({ initial, hasDraft: initialHasDraft }: Props) {
  const [draft, setDraft] = useState<OpusPassGuestsFaqsContent>(initial)
  ...
  const setField = <K extends keyof OpusPassGuestsFaqsContent>(
    key: K,
    value: OpusPassGuestsFaqsContent[K],
  ) => setDraft((d) => ({ ...d, [key]: value }))

  const setItem = (idx: number, patch: Partial<OpusPassGuestsFaqItem>) =>
    setDraft((d) => ({
      ...d,
  ...
  const handleSaveDraft = () =>
    runAction(async () => {
      await saveOpusPassGuestsFaqsDraft(draft)
      setHasDraft(true)
  ...
  const handlePublish = () =>
    runAction(async () => {
      await saveOpusPassGuestsFaqsDraft(draft)
      await publishOpusPassGuestsFaqs()
      setHasDraft(false)
  ...
  const handleDiscard = () =>
    runAction(async () => {
      await discardOpusPassGuestsFaqsDraft()
      setDraft(initial)
      setHasDraft(false)
