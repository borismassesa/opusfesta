...
import type {
  OpusPassGuestsTestimonialFg,
  OpusPassGuestsTestimonialItem,
  OpusPassGuestsTestimonialsContent,
...
import {
  discardOpusPassGuestsTestimonialsDraft,
  publishOpusPassGuestsTestimonials,
  saveOpusPassGuestsTestimonialsDraft,
} from './actions'

type Props = {
  initial: OpusPassGuestsTestimonialsContent
  hasDraft: boolean
}
...
function randomId(): string {
  return `gt-${Math.random().toString(36).slice(2, 9)}`
}

export default function TestimonialsEditor({ initial, hasDraft: initialHasDraft }: Props) {
  const [draft, setDraft] = useState<OpusPassGuestsTestimonialsContent>(initial)
  ...
  const setField = <K extends keyof OpusPassGuestsTestimonialsContent>(
    key: K,
    value: OpusPassGuestsTestimonialsContent[K]
  ) => setDraft((d) => ({ ...d, [key]: value }))

  const setItem = (col: Column, idx: number, patch: Partial<OpusPassGuestsTestimonialItem>) =>
    setDraft((d) => ({
      ...d,
    ...
    setDraft((d) => ({
      ...
      [col]: [
        ...d[col],
        {
          ...
          avatar: '',
          bg: 'bg-white',
          fg: 'light' as OpusPassGuestsTestimonialFg,
        },
      ],
    }))
  ...
  const handleSaveDraft = () =>
    runAction(async () => {
      await saveOpusPassGuestsTestimonialsDraft(draft)
      setHasDraft(true)
  ...
  const handlePublish = () =>
    runAction(async () => {
      await saveOpusPassGuestsTestimonialsDraft(draft)
      await publishOpusPassGuestsTestimonials()
      setHasDraft(false)
  ...
  const handleDiscard = () =>
    runAction(async () => {
      await discardOpusPassGuestsTestimonialsDraft()
      setDraft(initial)
      setHasDraft(false)
      ...
      {draft[col].map((item, idx) => (
        ...
        >
          ...
          <ImageUploadField
            ...
            pathPrefix="opus-pass/guests-testimonials"
...
function TestimonialCardMini({
  ...
  t: OpusPassGuestsTestimonialItem
  index: number
  locale: Locale
}) {
  ...
  return (
    <div className={`rounded-lg p-2 ${isDark ? 'bg-[#1A1A1A]' : 'bg-[#9FE870]'}`}>
      ...
      <p className={`text-[9px] font-semibold leading-snug ${text} line-clamp-3 mb-1`}>
