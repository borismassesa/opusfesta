...
import type { OpusPassInvitationsFaqsContent } from '@/lib/cms/opus-pass-invitations-faqs'

const PAGE_KEY = 'opus-pass-invitations'
const SECTION_KEY = 'faqs'

export async function saveOpusPassInvitationsFaqsDraft(
  draft: OpusPassInvitationsFaqsContent,
): Promise<void> {
  ...
  revalidatePath('/cms/opus-pass/invitations/faqs')
}

export async function publishOpusPassInvitationsFaqs(): Promise<void> {
...
export async function discardOpusPassInvitationsFaqsDraft(): Promise<void> {
  await requirePermission('cms.write')
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
