...
export async function saveOpusPassInvitationsHeroDraft(
  draft: OpusPassInvitationsHeroContent,
): Promise<void> {
  ...
  revalidatePath('/cms/opus-pass/invitations/hero')
}

export async function publishOpusPassInvitationsHero(): Promise<void> {
  const supabase = createSupabaseAdminClient()
...
export async function discardOpusPassInvitationsHeroDraft(): Promise<void> {
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
