...
const SECTION_KEY = 'free-website-promo'

export async function saveOpusPassInvitationsFreeWebsitePromoDraft(
  draft: OpusPassInvitationsFreeWebsitePromoContent,
): Promise<void> {
  ...
  revalidatePath('/cms/opus-pass/invitations/free-website-promo')
}

export async function publishOpusPassInvitationsFreeWebsitePromo(): Promise<void> {
  const supabase = createSupabaseAdminClient()
...
export async function discardOpusPassInvitationsFreeWebsitePromoDraft(): Promise<void> {
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
