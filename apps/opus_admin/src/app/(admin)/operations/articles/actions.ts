...
// 100 MB hard cap for videos / 25 MB for images. Sanity check; the
// contributor API route enforces the same so the limit holds for both
// upload paths.
const ADVICE_MEDIA_MAX_IMAGE_SIZE = 25 * 1024 * 1024
const ADVICE_MEDIA_MAX_VIDEO_SIZE = 100 * 1024 * 1024
...
export async function uploadAdviceMedia(formData: FormData): Promise<{ url: string; type: 'image' | 'video' }> {
  ...
  if (file.size > max) {
    throw new Error(
      isVideo ? 'Videos must be 100MB or smaller.' : 'Images must be 25MB or smaller.'
    )
  }
