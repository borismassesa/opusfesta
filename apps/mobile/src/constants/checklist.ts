// Mirrors the web checklist template (apps/opus_website/src/app/my/planning/PlanningClient.tsx
// SECTIONS) so mobile and web show the same tasks. Per the "match web" persistence
// decision: no wedding_tasks table — regular tasks are checked off locally
// (AsyncStorage, see useChecklist.ts) and "widget" tasks derive their completion
// from couple_profiles, same as the web client.
...
export const CHECKLIST_TOTAL_TASKS = CHECKLIST_SECTIONS.reduce((sum, s) => sum + s.tasks.length, 0);
