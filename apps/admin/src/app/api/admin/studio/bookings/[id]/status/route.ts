...
import { StudioBookingStatusSchema, createAdminAuditEvent } from "@/lib/studio-data";
...
export async function PATCH(
  ...
  return withStudioRole(request, "staff", async ({ actor }) => {
    ...
    const { data: existing } = await supabase.from("studio_bookings").select("*").eq("id", id).single();
    if (!existing) {
