    ...
    <section ref={containerRef} className="py-10 md:py-16 px-4 md:px-6 lg:px-10 bg-background flex justify-center">
      <div ref={cardRef} className="relative w-full max-w-[1200px] rounded-3xl md:rounded-[32px] overflow-hidden min-h-[360px] md:min-h-[480px] flex flex-col items-center justify-center text-center p-6 md:p-10 lg:p-14 shadow-xl">
        ...
        <div className="relative z-10 flex flex-col items-center max-w-4xl mx-auto">
          ...
          <p className="text-sm sm:text-base md:text-lg text-zinc-200 max-w-2xl leading-relaxed mb-6 md:mb-8 font-light px-3 line-clamp-3">
          ...
          <div className="flex flex-row gap-3 sm:gap-4 items-center justify-center w-full sm:w-auto cta-button-group px-2 sm:px-0">
            <Link
              ...
              className="w-auto sm:w-auto inline-flex items-center justify-center px-4 sm:px-5 md:px-6 py-3 md:py-3.5 rounded-full bg-white text-black font-semibold text-sm sm:text-base transition-all hover:bg-zinc-200 shadow-[0_0_32px_-12px_rgba(255,255,255,0.5)] whitespace-nowrap"
            >
              {cta?.primaryButtonText || "Get Started"}
            </Link>
            <Link
              href={cta?.secondaryButtonLink || "/demo"}
              className="w-auto sm:w-auto inline-flex items-center justify-center px-4 sm:px-5 md:px-6 py-3 md:py-3.5 rounded-full border border-white/50 bg-white/10 text-white font-semibold text-sm sm:text-base transition-all hover:bg-white/15 whitespace-nowrap"
            >
              {cta?.secondaryButtonText || "Live Demo"}
            </Link>
          </div>
          
          <div className="mt-6 md:mt-8 flex flex-col sm:flex-row items-center gap-3 sm:gap-4 cta-trust">
             <div className="rounded-full border border-white/30 bg-white/10 px-3 py-1.5 text-[10px] sm:text-xs text-white uppercase tracking-widest font-medium">
