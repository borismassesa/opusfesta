  ...
  return (
    <li className={cn({ 'h-19.5': description && splitItems })}>
      <NavigationMenuLink href={href} className={cn({ 'flex flex-row items-start gap-2': icon })}>
        {icon && (
          <span className='bg-muted/40 [&>svg]:!text-foreground flex aspect-square size-7.5 shrink-0 items-center justify-center rounded-lg border-0 [&>svg]:!size-4.5'>
            {icon}
          </span>
        )}
        {description ? (
        ...
        {navigationData.map(navItem => {
          ...
          return (
            <NavigationMenuItem key={navItem.title}>
              ...
              <NavigationMenuContent className='rounded-2xl border border-border/70 bg-background p-4 shadow-sm'>
                {navItem.splitItems ? (
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <PrimaryOrionButton size='icon-lg' className={cn('inline-flex lg:hidden', triggerClassName)}>
          <MenuIcon />
