...
const ProductInsightsCard = ({ className }: { className?: string }) => {
  return (
    <Card
      className={cn(
        "gap-4 rounded-2xl border-0 bg-background shadow-[0_2px_10px_-2px_rgba(0,0,0,0.05)] transition-all duration-300 hover:shadow-[0_8px_20px_-4px_rgba(0,0,0,0.1)]",
        className
      )}
    >
      <CardHeader className="flex justify-between">
        <div className="flex flex-col gap-1">
          <span className="text-lg font-semibold">Marketplace insights</span>
          <span className="text-muted-foreground text-sm">Updated Feb 6, 2026 · 9:10 AM</span>
        </div>
        <img
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-1">
          <div className="flex flex-col gap-1">
            <span className="text-xs">Inquiries sent</span>
