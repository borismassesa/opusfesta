...
interface DOMMotionProps<T extends HTMLElement = HTMLElement>
  extends Omit<HTMLMotionProps<keyof HTMLElementTagNameMap>, 'ref'> {
  ref?: React.Ref<T>
}
...
function Slot<T extends HTMLElement = HTMLElement>({ children, ref, ...props }: SlotProps<T>) {
  ...
  const childType = childElement.type as React.ElementType
