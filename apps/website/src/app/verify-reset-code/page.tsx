...
export default function VerifyResetCode() {
  ...
  const handleVerify = async (verificationCode?: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(formatError(data.error || "Invalid reset code"));
        setIsLoading(false);
        return;
      }
      ...
      // If we have a recovery link, extract tokens from it
      else if (data.recoveryLink) {
        try {
          const url = new URL(data.recoveryLink);
          ...
          if (accessToken && refreshToken) {
            ...
            if (sessionError) {
              console.error("Error setting session from recovery link:", sessionError);
              setError("Failed to create reset session. Please try again.");
              setIsLoading(false);
              return;
            }
          } else {
            // If we can't extract tokens, redirect to the recovery link
            // This will let Supabase handle the session creation
            window.location.href = data.recoveryLink;
            return;
          }
        } catch (urlError) {
          console.error("Error parsing recovery link:", urlError);
          // Redirect to recovery link as fallback
          window.location.href = data.recoveryLink;
          return;
        }
      }
      ...
      <div className={`w-full max-w-md space-y-4 transition-all duration-500 ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
        ...
        <div className="text-center space-y-1">
          <h1 className="text-3xl font-semibold tracking-tight text-primary">
            {isVerified ? "Code Verified!" : "Enter reset code"}
          </h1>
          {!isVerified && (
        ...
        {!isVerified && (
          <div className="space-y-3 animate-in fade-in duration-300">
            {/* OTP Input */}
            <div className="flex justify-center" role="group" aria-label="Password reset code input">
              <InputOTP
                maxLength={6}
                value={code}
                onChange={(value) => {
            ...
            <div className="text-center space-y-1 pt-2">
              ...
              <button
                ...
                aria-label={resendCooldown > 0 ? `Resend code in ${resendCooldown} seconds` : "Resend reset code"}
              >
                {isResending ? (
                  <>
        ...
        {isVerified && (
          <div className="text-center space-y-2 animate-in fade-in duration-300">
            <p className="text-sm text-muted-foreground">
              Your code has been verified successfully. Redirecting you to reset your password...
            </p>
          </div>
        )}
      </div>
