...
function PostNavigation({ currentPost }: { currentPost: AdviceIdeasPost }) {
  const sortedPosts = [...adviceIdeasPosts].sort((a, b) => a.id - b.id)
...
export default async function AdviceIdeasSlugPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const post = adviceIdeasPosts.find(p => p.slug === slug)

  if (!post) {
    notFound()
  }

  const sameCategoryPosts = adviceIdeasPosts.filter(p => p.category === post.category && p.slug !== post.slug)
  const otherPosts = adviceIdeasPosts.filter(p => p.category !== post.category && p.slug !== post.slug)
