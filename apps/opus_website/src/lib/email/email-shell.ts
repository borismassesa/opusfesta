// Shared chrome for every transactional email the opus_website sends.
// Mirrors apps/vendors_portal/src/lib/email/email-shell.ts so brand colors
// stay consistent. The footer copy is adapted for client-facing emails.
...
export function bulletproofButton(args: { href: string; label: string }): string {
    ...
    <tr>
      <td style="padding:28px 32px 20px;border-top:1px solid ${BRAND.ink.line};">
        ...
        <p style="margin:6px 0 0;font-size:12px;line-height:1.65;color:${BRAND.ink.muted};">
          You received this because you submitted an inquiry or have an OpusFesta account.
          This is an automated message — replies route to a human if applicable.
        </p>
      </td>
    </tr>`
}
