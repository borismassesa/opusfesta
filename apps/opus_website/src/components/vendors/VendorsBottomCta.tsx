...
export default function VendorsBottomCta() {
  ...
  return (
    <section className="max-w-[1200px] mx-auto px-5 sm:px-8 lg:px-10 pt-20 sm:pt-28 pb-12 sm:pb-16">
      {/* ── Heading ── */}
      <motion.h2
        className="text-[1.75rem] sm:text-[2rem] lg:text-[2.5rem] font-bold text-center text-(--foreground) tracking-tight leading-tight mb-14 sm:mb-20"
        {...fadeUp(0)}
      >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 sm:gap-8 lg:gap-14 mb-14 sm:mb-20">
        {steps.map((step, i) => (
          ...
          >
            ...
            <p className="text-[0.84rem] leading-relaxed text-(--foreground)/60 max-w-[280px]">
              {step.description}
            </p>
          </motion.div>
        ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-6 lg:gap-8">
        {cards.map((card, i) => (
          <motion.div
            ...
            {...fadeUp(0.08 + i * 0.1)}
          >
            <h3 className="text-lg sm:text-xl font-bold text-(--foreground) leading-snug mb-2.5">
              {card.title}
            </h3>
            <p className="text-[0.84rem] leading-relaxed text-(--foreground)/60 mb-7 max-w-[260px]">
              {card.description}
            </p>
            <Link
              href={card.href}
              className="inline-block bg-(--accent) hover:bg-(--accent-hover) text-[#1A1A1A] px-6 py-2.5 rounded-full font-semibold text-sm transition-colors"
            >
              {card.cta}
            </Link>
          </motion.div>
