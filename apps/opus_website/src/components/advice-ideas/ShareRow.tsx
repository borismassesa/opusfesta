...
export default function ShareRow({
  ...
  const copyLink = async () => {
    try {
      ...
      // Older Safari iOS doesn't allow clipboard.writeText without https.
      // Quietly no-op rather than throw.
    }
  }
  ...
  const buttonClass =
    'flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-600 transition-colors hover:border-[var(--accent-hover)] hover:text-[var(--accent-hover)]'

  return (
    <div
      className={`flex flex-wrap items-center gap-2 ${
        variant === 'block' ? 'rounded-2xl bg-[#FAF7F2] px-5 py-4' : ''
      }`}
    >
      ...
      <button
        ...
        className={`${buttonClass} ${
          liked ? 'border-[var(--accent-hover)] text-[var(--accent-hover)]' : ''
        }`}
      >
        <Heart
          size={16}
      ...
      <span
        aria-live="polite"
        className={`text-[12px] font-semibold text-[var(--accent-hover)] transition-opacity ${
