...
export default function AuthorCard({ author }: { author: AdviceIdeasAuthor }) {
  ...
  return (
    <aside className="flex flex-col gap-5 rounded-2xl border border-gray-200 bg-white p-6 sm:flex-row sm:items-start sm:p-7">
      <div
        aria-hidden
        className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-[#FAF7F2] text-[15px] font-bold tracking-wide text-[var(--accent-hover)] sm:h-16 sm:w-16 sm:text-[16px]"
      >
        {author.initials}
      </div>
